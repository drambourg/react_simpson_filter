...
class App extends Component {
  constructor(props) {
    ...
    this.handleShowSimpsonsOnly = this.handleShowSimpsonsOnly.bind(this);
  }

  handleShowSimpsonsOnly() {
    this.setState({ showSimpsonOnly: !this.state.showSimpsonOnly });
  }
    ...
    return (
      <div>
        <button onClick={this.handleShowSimpsonsOnly}>
          {this.state.showSimpsonOnly
            ? "Show Only Simpson Characters"
            : "Show All characters"}
        </button>
        {this.state.quotes
          .filter(
            quote =>
              this.state.showSimpsonOnly ||
              quote.character.toLowerCase().includes("simpson")
          )
          .map((quote, index) => (
            <Quote {...quote} />
          ))}
